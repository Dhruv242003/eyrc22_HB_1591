...
global error_x, error_y, vel_x , vel_y, vel_z
...
sub1 = rospy.Subscriber('task1_goals', PoseArray, task1_goals_Cb)	
	...
	if(len(x_goals)==5):
		...
		while True:
			...
			if ((theta_goals[i]>=0 and theta_goals[i]<1.570795) or (theta_goals[i]<=0 and theta_goals[i]>-1.570795)):
				...
				vel.angular.z=vel_z
				pub.publish(vel)
				if(distance>0.0 and distance<0.035):
					vel.linear.x=0.0
					vel.linear.y=0.0
					pub.publish(vel)
					if((desired_error>=0.0 and desired_error<0.035) or (desired_error<0.0 and desired_error>=-0.035)):
						vel.angular.z=0.0
						pub.publish(vel)
						rospy.sleep(1)
						break
				...
				if(distance>0.0 and distance<0.04):
					vel.linear.x=0.0
					vel.linear.y=0.0
					pub.publish(vel)
					if((desired_error>=0.0 and desired_error<0.035) or (desired_error<0.0 and desired_error>=-0.035)):
